package ncount

import (
	"encoding/json"
	"fmt"
)

/*
version 版本号 5 目前必须为 1.0 不可 例如：1.0
tranCode 交易代码 5 此交易只能为 R010 不可 例如：R010
merId 商户ID 12 新账通平台提供给商户 的唯一 ID 不可 例 如 ： 00000100000
merOrderId 商 户 订 单 号 1 - 32 格式：数字，字母，下 划线，竖划线，中划线 不可 例 如 ： aa201612011 102
submitTime 请 求 提 交 时间 14 格 式 ： YYYYMMDDHHMMS S 不可 例 如 ： 20161201110 233
msgCiphert ext 报文密文 1-4000 用平台公钥 RSA 加密后 base64 的编码值 不可
signType 签名类型 1 1：RSA 不可 例如：1
signValue 签 名 密 文 串 将 报 文 信 息 用
signType 域设 置的方 式签名后生成的字符串 不可
merAttach 附加数据 0-80 格式：英文字母/汉字 可空
charset 编码方式 1 1：UTF8 不可 例如：1长度(类 型均为 String) 参数说明 是否可空 样例
merUserId 商 户 用户 唯 一标识 1-30 按照商户侧规则 同一个平台商户唯一 字符串类型 不可 例 如 ： xsqianyi1_ 148080610 02
mobile 用户手机号 11 用户本人在运营商已 实名的手机号 不可 例如： 138000000 00
userName 真实姓名 1-30 和身份证上的姓名保 持一致 不可 例如：张三
certNo 身份证号 18 身份证号码 不可 例如： 110210199 008123456
*/
type NCountAccountReq struct {
	version    string `json:"version" binding:"required"`
	tranCode   string `json:"tranCode" binding:"required"`
	merId      string `json:"merId" binding:"required"`
	merOrderId string `json:"merOrderId" binding:"required"`
	submitTime string `json:"submitTime" binding:"required"`
	msgCiphert string `json:"msgCiphert" binding:"required"`
	signType   string `json:"signType" binding:"required"`
	signValue  string `json:"signValue" binding:"required"`
	merUserId  string `json:"merUserId" binding:"required"`
	mobile     string `json:"mobile" binding:"required"`
	userName   string `json:"userName" binding:"required"`
	certNo     string `json:"certNo" binding:"required"`
	charset    string `json:"charset" binding:"required"`
}

/*
	version=[]tranCode=[]merId=[]merOrderId=[]submitTime=[]msgCiphertext=[]signType=[] 注：
	1. 中括号“[]”里面为各字段对应的值；
	2. RSA 签名时，商户使用商户私钥进行签名；
	3. 签名后的二进制转 Base64 编码。
*/
func (req *NCountAccountReq) marshal() ([]byte, error) {
	var msgCipher = make(map[string]string)
	msgCipher["merUserId"] = req.merUserId
	msgCipher["mobile"] = req.mobile
	msgCipher["userName"] = req.userName
	msgCipher["certNo"] = req.certNo
	data, err := json.Marshal(msgCipher)
	fmt.Println(data)
	if err != nil {
		return nil, err
	}
	return nil, err
}

func (req *NCountAccountReq) getUrl() (string, error) {
	url := ""
	url += "version=[ " + req.version + " ]"
	url += "tranCode=[ " + req.tranCode + " ]"
	url += "merId=[ " + req.merId + " ]"
	url += "merOrderId=[ " + req.merOrderId + " ]"
	url += "submitTime=[ " + req.submitTime + " ]"
	url += "msgCiphertext=[ " + req.msgCiphert + " ]"
	url += "signType=[ " + req.signType + " ]"
	return url, nil
}

/*
	version 版本号 同上送 tranCode 交易代码 同上送 merOrderId 商户订单号 同上送 merId 商户 ID 同上送
	merAttach 附加数据 同上送
	charset 编码方式 同上送
	signType 签名类型 同上送
	resultCode 处理结果码 4 详情参见附录二
	resultCode 9999
	errorCode 异常代码 1-10 详情参见附录一
	errorCode errorMsg 异常描述 1-200 中文、字母、数字
	userId 用户编号 12 用户编号
	signValue 签名字符串 将报文信息用
	signType 域设 置的方式签名后生成的字符 串
*/

type NewNCountAccountResp struct {
	version    string `json:"version" binding:"required"`
	merAttach  string `json:"merAttach" binding:"required"`
	charset    string `json:"charset" binding:"required"`
	resultCode string `json:"resultCode" binding:"required"`
	errorCode  string `json:"errorCode" binding:"required"`
	errorMsg   string `json:"errorMsg" binding:"required"`
	userId     string `json:"userId" binding:"required"`
	signValue  string `json:"signValue" binding:"required"`
	signType   string `json:"signtype" binding:"required"`
}

func (c *counter) NewNCounterAccount(req *NCountAccountReq) (*NewNCountAccountResp, error) {
	url, err := req.getUrl()
	if err != nil {
		return nil, err
	}
	data, err := req.marshal()
	if err != nil {
		return nil, err
	}
	body, err := httpPost(url, data)
	if err != nil {
		return nil, err
	}
	var respData NewNCountAccountResp
	err = json.Unmarshal(body, &respData)
	if err != nil {
		return nil, err
	}
	return &respData, nil
}
